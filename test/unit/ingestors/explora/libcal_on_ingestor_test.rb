...
class LibcalOnIngestorTest < ActiveSupport::TestCase
  # Npte: we just use the BC data, but pretend it's from an Ontario provider
  # Local times will be the same, UTC times will be three hours different
  setup do
  ...
  test 'Ingest libcal ON source' do
    # override time
    assert_difference('Event.count', 3) do
      freeze_time(2019) do
        ingestor = Ingestors::Explora::LibcalOnIngestor.new
        source = @content_provider.sources.build(
          url: 'https://www.libcal-bc.ca/ajax/calendar/list?c=7544&date=0000-00-00&cats=33865',
          method: 'libcal_event_on', enabled: true
        )
        ...
        assert_equal(event.timezone, 'Eastern Time (US & Canada)')
        assert_equal(event.start_local.to_s, '2025-06-02 12:30:00 -0400')
        assert_equal(event.end_local.to_s, '2025-06-02 13:00:00 -0400')
        ...
        assert_equal(event.presence, 'online')
        assert_equal(event.timezone, 'Eastern Time (US & Canada)')
        assert_equal(event.start_local.to_s, '2025-06-03 12:30:00 -0400')
        assert_equal(event.end_local.to_s, '2025-06-03 13:00:00 -0400')
        ...
        assert_equal(event.timezone, 'Eastern Time (US & Canada)')
        assert_equal(event.start_local.to_s, '2025-06-04 11:00:00 -0400')
        assert_equal(event.end_local.to_s, '2025-06-04 13:00:00 -0400')
      end
    end
  end
